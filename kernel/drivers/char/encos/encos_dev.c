...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Cookie Sun Devil");
...
// hacky way to emulate this output Lmao
// Todo: dynamically create multiple debugfs files and output buffers 
// Todo: dynamically allocated at enclave createion
#define ENCOS_DEBUGFS_DIR     "encos-IO-emulate"
#define ENCOS_DEBUGFS_SZ      0x1000000
static struct dentry *dir;
char emulated_output_buf[ENCOS_DEBUGFS_SZ] __attribute__((aligned(0x1000)));

static long encos_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    int rvl;
    int enc_pid, enc_pgrp, enc_id;
    struct encos_payload io_payload;
    rvl = 0;
    ...
    switch (cmd) {
        case ENCOS_ENCLAVE_REQUEST:
            ...
            enc_id = SM_encos_enclave_assign(emulated_output_buf);
        ...
        // --------------------------------
        // TO: finish copy_user() in monitor and handle them in the monitor
        case ENCOS_INPUT_PAYLOAD:
            if (copy_from_user(&io_payload, (void *)arg, sizeof(struct encos_payload))) {
                log_err("Failed to copy input payload.\n");
                return -EFAULT;
            }
            // copy kernel's emulated_output_buf back to payload buffer
            // calculate emulated_output_buf's string size and truncate
            if (io_payload.size > strlen(emulated_output_buf)) {
                io_payload.size = strlen(emulated_output_buf);
            }
            if (copy_to_user(io_payload.buf, emulated_output_buf, io_payload.size)) {
                log_err("Failed to copy-back input payload content.\n");
                return -EFAULT;
            }
            break;

        case ENCOS_OUTPUT_PAYLOAD:
            if (copy_from_user(&io_payload, (void *)arg, sizeof(struct encos_payload))) {
                log_err("Failed to copy input payload.\n");
                return -EFAULT;
            }
            // copy user content to the kernel buffer
            if (io_payload.size > ENCOS_DEBUGFS_SZ) {
                io_payload.size = ENCOS_DEBUGFS_SZ;
            }
            if (copy_from_user(emulated_output_buf, io_payload.buf, io_payload.size)) {
                log_err("Failed to copy input payload content.\n");
                return -EFAULT;
            }
            break;
    }
...
static int encos_mmap(struct file *file, struct vm_area_struct *vma)
{
    ...
    /* 
     * Deprecated below. Let's simply use OS's anonymous backend for SHM 
     * and just remove the Perm(RD). 
     */
    else {/* NOT USED! */
...
static ssize_t debugfs_memory_read(struct file *file, char __user *buf, size_t count, loff_t *pos) {
    // Read memory into user buffer
    size_t size_to_copy;
    
    if (*pos >= ENCOS_DEBUGFS_SZ) {
        return 0;  // End of file
    }

     size_to_copy = min(count, (size_t)(ENCOS_DEBUGFS_SZ - *pos));

    if (size_to_copy == 0) {
        return 0;  // End of file
    }

    if (copy_to_user(buf, emulated_output_buf + *pos, size_to_copy)) {
        return -EFAULT;
    }
...
static ssize_t debugfs_memory_write(struct file *file, const char __user *buf, size_t count, loff_t *pos) {
    // Write memory from user buffer
    size_t size_to_copy;
    
    if (*pos >= ENCOS_DEBUGFS_SZ) {
        return -EINVAL;  // End of file
    }
    ...
    if (copy_from_user(emulated_output_buf + *pos, buf, size_to_copy)) {
        return -EFAULT;
    }
...
static const struct file_operations debugfs_fops = {
    .owner = THIS_MODULE,
    .read = debugfs_memory_read,
    .write = debugfs_memory_write,
};
...
static int __init encos_dev_init(void)
{
    ...
    dir = debugfs_create_dir(ENCOS_DEBUGFS_DIR, NULL);
    if (!dir) {
        pr_err("Failed to create debugfs directory\n");
        return -ENOMEM;
    }
    // rw-rw-rw-
    if (!debugfs_create_file("out", 0666, dir, NULL, &debugfs_fops)) {
        pr_err("Failed to create debugfs file\n");
        return -ENOMEM;
    }
    if (!debugfs_create_file("in", 0666, dir, NULL, &debugfs_fops)) {
        pr_err("Failed to create debugfs file\n");
        return -ENOMEM;
    }
