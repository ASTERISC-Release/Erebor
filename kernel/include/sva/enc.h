...
/* Chuqi:
 * The following interfaces are exposed to arch/x86/entry/entry_64.S
...
.extern SM_do_syscall(struct pt_regs *regs, int nr);
...
typedef struct encos_enclave_entry {
    ...
    /*
     * Use a debugfs memory to emulate the data processing output channel.
     * Note this is untrusted and only for evaluation/illustration purpose.
     */
    char *emulate_output_buf;
    /* Chuqi:
...
extern int SM_encos_enclave_assign(char *debugfs_output_mem);
