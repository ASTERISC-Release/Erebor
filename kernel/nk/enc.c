...
SECURE_WRAPPER(int, 
SM_encos_enclave_assign, char *debugfs_output_mem)
{
#ifdef CONFIG_ENCOS_STATS
    ...
    encos_enclave_table[pid].emulate_output_buf = debugfs_output_mem;
...
int is_internalmem)
{
    ...
    encos_enclave_entry_t *entry = current_enclave_entry();
    /* set up its CR3 */
    SVA_ASSERT(entry->enc_CR3, "The current enclave (sandbox) did not declare its CR3.\n");
...
unsigned int size)
{
    ...
    /* mask and restore sensitive information */
...
// ioctl(fd, 0xBAC0, (input_payload *)input_payload);
// ioctl(fd, 0xBAC1, (output_payload *)output_payload);

SECURE_WRAPPER(int, SM_do_syscall,
struct pt_regs *regs,
int nr)
{
    encos_enclave_entry_t *entry = current_enclave_entry();
    // unsigned long arg0 = regs->di;
    unsigned long arg1 = regs->si;
    unsigned long arg2 = regs->dx;

    /* handle ioctl() for I/O */
    // printk("Intercept syscall[%d]. arg0=0x%lx, arg1=0x%lx.\n", nr, arg0, arg1);

    if (!(nr == 16 && (arg1 >= 0xBABE && arg1 <= 0xBAC1)))
        goto filter;

    switch (arg1)
    {
    case 0xBABF:
        memcpy(entry->emulate_output_buf, &arg2, sizeof(unsigned long));
        break;
    
    // Finish copy_user() support and handle pointer-based payload here
    default:
        return 0;
    }
    return 0x9999BABE;
    ...
    /*
     * For debug and benchmark purpose, let's not 
     * filter syscalls for the sandbox and let them 
     * be able to print software stats.
     */
filter:
    if (entry->activate) { /*eval*/ };
    return 0;
}
